...
@php
    ...
    $variantStyles = match($this->variant) {
    ...
    $disabledStyles = ($this->disabled || $this->loading) ? 'opacity: 0.5; cursor: not-allowed;' : '';
...
@if($this->href)
    <a
        href="{{ $this->href }}"
        style="{{ $allStyles }}"
        @if($this->disabled) aria-disabled="true" tabindex="-1" @endif
    >
        @if($this->loading)
        ...
        @elseif($this->icon && $this->iconPosition === 'left')
            <span style="margin-right: 0.5rem;">{!! $this->icon !!}</span>
        @endif

        {{ $this->label }}

        @if($this->icon && $this->iconPosition === 'right' && !$this->loading)
            <span style="margin-left: 0.5rem;">{!! $this->icon !!}</span>
        @endif
    </a>
@else
    <button
        type="{{ $this->type }}"
        style="{{ $allStyles }}"
        @if($this->disabled || $this->loading) disabled @endif
    >
        @if($this->loading)
            <svg style="width: 1rem; height: 1rem; margin-right: 0.5rem; animation: spin 1s linear infinite;" fill="none" viewBox="0 0 24 24">
                <circle style="opacity: 0.25;" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
